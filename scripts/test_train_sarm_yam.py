...
def test_data_configuration():
    action_horizon = 42
    yam_config = LeRobotYamDataConfig(
    ...
    data_config = yam_config.create(assets_dirs=Path('/'), model_config=pi0_config.Pi0Config())
    ...
    assert 'observation.images.right_wrist' in frame
    ...
    image_map = {'observation.images.right_wrist': 'right_wrist_0_rgb',
                 'observation.images.left_wrist': 'left_wrist_0_rgb',
                 'observation.images.topdown': 'base_0_rgb',
                 }
    ...
    for n in range(action_horizon):
        np.testing.assert_almost_equal(frame_transformed['actions'][n][:6]+frame_transformed['state'][:6], frame['action'][n][:6])
        np.testing.assert_almost_equal(frame_transformed['actions'][n][7:13]+frame_transformed['state'][7:13], frame['action'][n][7:13])
        # Grippers
...
def test_data_configuration_with_normalization():
    repo_id = 'ETHRC/towel_base'
    train_config_id = 'pi0_yam_debug'
    assets_dir = NORM_STATS_PATH / train_config_id
    ...
    data_config = yam_config.create(assets_dirs=assets_dir, model_config=pi0_config.Pi0Config())
    lerobot_dataset = create_torch_dataset(data_config=data_config, action_horizon=action_horizon, model_config=None)
    dataset_transformed = transform_dataset(lerobot_dataset, data_config, skip_norm_stats=True)
...
@pytest.mark.parametrize("config_name", ["pi0_yam_debug"])
...
def test_get_reward_data():
    action_horizon = 42
    piper_config = LeRobotYamSarmDataConfig(
        repo_id='ETHRC/towel_base',
        assets=AssetsConfig(
            assets_dir="",
            asset_id=None,
